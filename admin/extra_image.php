               ...
               <div class="col-md-4">
                  <div class="panel-primary">
                     ...
                     <div class="panel-body">
                        ...
                        ?>
                        <form action="action/manage_extra_image.php" method="POST" enctype="multipart/form-data">
                           <div class="form-group">
                              <label for="">Select Product</label>
                              <select class="form-select" name="p_id" aria-label="">
                                 <option selected>Select</option>
                                 <?php
                                 $product = $db->query("SELECT * FROM `product`");
                                 while ($product_value = $product->fetch_object()) {
                                 ?>
                                    <option <?php
                                             if (!empty($_REQUEST['edit'])) {
                                                if (!empty($edit_value->p_id)) {
                                                   if ($edit_value->p_id === $product_value->p_id) {
                                                      echo "selected";
                                                   }
                                                }
                                             }
                                             ?> value='<?= $product_value->p_id; ?>'> <?= $product_value->product_name; ?> </option>
                                 <?php } ?>
                              </select>
                           </div>
                           <div class="form-grp">
                                 ...
                                 <div class="col-md-12">
                                    <div class="table-responsive">
                                       <table class="table table-bordered table-striped" style="width:100%">
                                          ...
                                          <tbody>
                                             <?php
                                             $data = $db->query("SELECT * FROM `product_extra_image` LEFT JOIN `product` ON product_extra_image.pei_id = product.p_id");
                                             if ($data->num_rows == 0) {
                                                echo 'No Data Found';
                                             } else {
                                                $sl = 0;
                                             ...
                                             ?>
                                                   <tr>
                                                      ...
                                                      <td>
                                                         ...
                                                         <a href="action/manage_extra_image.php?submit=delete&pei_id=<?= $value->pei_id; ?>" class="btn btn-danger"><i class="fa-solid fa-trash"></i></a>
                                                      </td>
